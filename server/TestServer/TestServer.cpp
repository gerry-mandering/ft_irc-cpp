...
int main()
{
    ...
    int server_socket;  //Connection Listening 소켓 fd
    struct sockaddr_in server_addr; //address 정보

    if ((server_socket = socket(PF_INET, SOCK_STREAM, 0)) == -1)    //AF_INET이 표준, IPv4와 스트림 소, 프로토콜 -> getaddrinfo 후에 소켓 생성방식 ㄱ
    ...
    change_events(change_list, server_socket, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL); //ENABLE 적을 필요 없음
